export function decimalToBigInt(decimalStr: string, decimalPlaces: number): bigint {
    // Check if the string contains a decimal point
    const parts = decimalStr.split(".");
    let integerPart = parts[0]; // part before the decimal
...
export function bigIntToDecimalString(bigIntAsStr: string, decimals: number): string {
